...
interface Game {
  ...
  winner: string; mvp: string | null; // "A" | "B" | "D" (draw)
  player_scores: Record<string, number>;
  game_type?: string; // 'casual' | 'official'
}
interface Duel {
  id: string; created_at: string;
...
function getTier(p: Player, games?: Game[]) {
  const g = p.wins + p.losses;
  if (g < 3) return null;
  const score = getTierScore(p, games) ?? 0;
  return TIERS.find(t => score >= t.min) ?? TIERS[TIERS.length - 1];
}
function getTierScore(p: Player, games?: Game[]) {
  const g = p.wins + p.losses;
  if (g < 3) return null;
  if (!games) return Math.round(p.win_rate * 0.7 + Math.min(p.mvp / 10, 1) * 100 * 0.3);
  // 공식전(5vs5) 가중치 더 높게
  const myGames = games.filter(g => [...(g.team_a||[]),...(g.team_b||[])].includes(p.id));
  const officialGames = myGames.filter(g => g.game_type === 'official');
  const casualGames = myGames.filter(g => g.game_type !== 'official');
  const oWins = officialGames.filter(g => { const inA=(g.team_a||[]).includes(p.id); return (inA&&g.winner==="A")||(!inA&&g.winner==="B"); }).length;
  const oTotal = officialGames.length;
  const cWins = casualGames.filter(g => { const inA=(g.team_a||[]).includes(p.id); return (inA&&g.winner==="A")||(!inA&&g.winner==="B"); }).length;
  const cTotal = casualGames.length;
  const oWr = oTotal > 0 ? (oWins/oTotal)*100 : 0;
  const cWr = cTotal > 0 ? (cWins/cTotal)*100 : 0;
  // 공식전MVP 1.5배, 일반MVP 1배
  const oMvp = games.filter(g => g.game_type==='official' && g.mvp===p.id).length;
  const cMvp = games.filter(g => g.game_type!=='official' && g.mvp===p.id).length;
  const mvpScore = Math.min((oMvp*1.5 + cMvp) / 10, 1) * 100;
  // 공식전 가중치 높음
  const wrScore = oTotal > 0 && cTotal > 0
    ? oWr * 0.6 + cWr * 0.4
    : oTotal > 0 ? oWr : cWr;
  return Math.round(wrScore * 0.7 + mvpScore * 0.3);
}
    ...
    <div style={S.root}>
      ...
      <nav style={S.nav}>
        <div style={S.navInner}>
          {[["home","홈"],["players","선수"],["record","기록"],["official","5vs5"],["duel","1vs1"],["log","경기"]].map(([k,v]) => (
      ...
      <main style={S.main}>
        ...
        {tab === "official"&& <OfficialGame players={players} games={games} onReload={load} onSelectPlayer={setSelectedPlayer} />}
        ...
        {form.length === 0 ? <Empty text="경기 기록 없음" /> : (
          <div style={{display:"flex",gap:8,alignItems:"center"}}>
            ...
            {[...form].reverse().map((r,i) => (
              <div key={i} style={{width:36,height:36,borderRadius:8,display:"flex",alignItems:"center",justifyContent:"center",background:r==="W"?"#FF6200":r==="D"?"#1e3a5f":"#161616",border:r==="W"||r==="D"?"none":"1px solid #2a2a2a",fontWeight:800,fontSize:13,color:r==="W"?"#fff":r==="D"?"#60A5FA":"#444"}}>{r==="W"?"승":r==="D"?"무":"패"}</div>
            ))}
          </div>
        )}
      </div>
    ...
    <div style={S.page}>
      <div style={S.summaryRow}>
        <div style={S.summaryCard}>
          <span style={S.summaryNum}>{games.filter(g=>g.game_type!=='official').length}</span>
          <span style={S.summaryLabel}>일반</span>
        </div>
        <div style={S.summaryDivider}/>
        <div style={S.summaryCard}>
          <span style={S.summaryNum}>{games.filter(g=>g.game_type==='official').length}</span>
          <span style={S.summaryLabel}>공식 5v5</span>
        </div>
        <div style={S.summaryDivider}/>
        <div style={S.summaryCard}>
...
function RecordGame({ players, onReload }: { players: Player[]; onReload: () => void }) {
  ...
  const [winner, setWinner] = useState<"A"|"B"|"D">("A");
  ...
  async function submit() {
    ...
    try {
      ...
      await sb.post("games",{id:uid(),score_a:Number(scoreA),score_b:Number(scoreB),team_a:teamA,team_b:teamB,winner,mvp:mvp||null,player_scores:scores,game_type:'casual'});
      const isDraw = winner === "D";
      const winTeam=isDraw?[]:winner==="A"?teamA:teamB;
      const loseTeam=isDraw?[]:winner==="A"?teamB:teamA;
      ...
      await Promise.all(players.map(async p=>{
        ...
        const inGame=[...teamA,...teamB].includes(p.id);
        if(!isDraw&&!isWin&&!isLose&&mvp!==p.id) return;
        if(isDraw&&!inGame&&mvp!==p.id) return;
      ...
      setDone(true);
      setTimeout(()=>{setDone(false);setScoreA("");setScoreB("");setTeamA([]);setTeamB([]);setGuestsA([]);setGuestsB([]);setMvp("");setPlayerScores({});setWinner("A");onReload();},1500);
    ...
    <div style={S.page}>
      ...
      <div style={S.card}>
        ...
        <div style={{marginBottom:14}}>
          <div style={S.fieldLabel}>결과</div>
          <div style={{display:"flex",gap:8}}>
            {(["A","B","D"] as const).map(t=>(
              <button key={t} onClick={()=>setWinner(t)} style={{flex:1,padding:"10px 0",borderRadius:8,border:`1px solid ${winner===t?"#FF6200":"#333"}`,background:winner===t?"rgba(255,98,0,0.15)":"transparent",color:winner===t?"#FF6200":"#555",cursor:"pointer",fontWeight:700,fontSize:13,transition:"all .15s"}}>
                {t==="D"?"무승부":`TEAM ${t} WIN`}
              </button>
            ))}
          </div>
...
// ── OFFICIAL 5v5 ──────────────────────────────────────────────────────────────
function OfficialGame({ players, games, onReload, onSelectPlayer }: { players: Player[]; games: Game[]; onReload: () => void; onSelectPlayer: (p:Player)=>void }) {
  const [scoreA, setScoreA] = useState("");
  const [scoreB, setScoreB] = useState("");
  const [teamA, setTeamA] = useState<string[]>([]);
  const [teamB, setTeamB] = useState<string[]>([]);
  ...
  const allTeamPlayers=inTeam.map(id=>players.find(p=>p.id===id)).filter(Boolean) as Player[];
  const [winner, setWinner] = useState<"A"|"B"|"D">("A");
  const officialGames = games.filter(g=>g.game_type==='official');
  ...
  async function submit() {
    ...
    try {
      const scores:Record<string,number>={};
      Object.entries(playerScores).forEach(([k,v])=>{if(v) scores[k]=Number(v);});
      await sb.post("games",{id:uid(),score_a:Number(scoreA),score_b:Number(scoreB),team_a:teamA,team_b:teamB,winner,mvp:mvp||null,player_scores:scores,game_type:'official'});
      const isDraw = winner === "D";
      const winTeam=isDraw?[]:winner==="A"?teamA:teamB;
      const loseTeam=isDraw?[]:winner==="A"?teamB:teamA;
      const allGames=await sb.get("games","?order=created_at.desc");
      ...
      setDone(true);
      setTimeout(()=>{setDone(false);setScoreA("");setScoreB("");setTeamA([]);setTeamB([]);setMvp("");setPlayerScores({});setWinner("A");onReload();},1500);
    } catch(e:any){setErr(e.message);}
    finally{setSaving(false);}
  }

  // 공식전 선수별 스탯
  const officialStats = players.map(p=>{
    const pg=officialGames.filter(g=>[...(g.team_a||[]),...(g.team_b||[])].includes(p.id));
    const wins=pg.filter(g=>{const inA=(g.team_a||[]).includes(p.id);return(inA&&g.winner==="A")||(!inA&&g.winner==="B");}).length;
    const losses=pg.length-wins;
    const wr=pg.length>0?Math.round(wins/pg.length*100):0;
    const mvpCnt=pg.filter(g=>g.mvp===p.id).length;
    const scoredG=pg.filter(g=>g.player_scores?.[p.id]!==undefined);
    const avg=scoredG.length>0?Math.round(scoredG.reduce((s,g)=>s+(g.player_scores[p.id]||0),0)/scoredG.length*10)/10:0;
    return {p,wins,losses,wr,mvpCnt,avg,games:pg.length};
  }).filter(x=>x.games>0).sort((a,b)=>b.wr-a.wr||b.games-a.games);
  ...
  return (
    <div style={S.page}>
      ...
      <div style={{...S.card,display:"flex",gap:0,padding:0,overflow:"hidden"}}>
        <button onClick={()=>setViewLog(false)} style={{flex:1,padding:"12px 0",fontSize:13,fontWeight:700,background:!viewLog?"#FF6200":"transparent",color:!viewLog?"#fff":"#555",border:"none",cursor:"pointer",borderBottom:`2px solid ${!viewLog?"#FF6200":"transparent"}`}}>기록 입력</button>
        <button onClick={()=>setViewLog(true)}  style={{flex:1,padding:"12px 0",fontSize:13,fontWeight:700,background:viewLog?"#FF6200":"transparent",color:viewLog?"#fff":"#555",border:"none",cursor:"pointer",borderBottom:`2px solid ${viewLog?"#FF6200":"transparent"}`}}>공식전 기록 ({officialGames.length})</button>
      </div>
      ...
      {!viewLog ? (
        <>
          ...
          {officialStats.length > 0 && (
            <div style={S.card}>
              <div style={S.cardHeader}>
                <span style={S.cardTitle}>공식전 순위</span>
                <span style={{fontSize:10,fontWeight:800,color:"#FF6200",background:"rgba(255,98,0,0.12)",padding:"2px 8px",borderRadius:4,letterSpacing:1}}>5v5</span>
              </div>
              {officialStats.map((x,i)=>(
                <div key={x.p.id} style={{...S.rankRow,cursor:"pointer"}} onClick={()=>onSelectPlayer(x.p)}>
                  <span style={{...S.rankIdx,color:i<3?"#FF6200":"#555"}}>{i+1}</span>
                  <div style={S.rankInfo}>
                    <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:5}}>
                      <span style={S.rankName}>{x.p.name}</span>
                      <span style={{fontSize:11,color:"#555"}}>{x.wins}W {x.losses}L</span>
                      {x.mvpCnt>0&&<span style={{fontSize:11,color:"#F59E0B",fontWeight:700}}>MVP×{x.mvpCnt}</span>}
                      {x.avg>0&&<span style={{fontSize:11,color:"#60A5FA",fontWeight:700}}>{x.avg}pts</span>}
                    </div>
                    <div style={S.rankBar}><div style={{...S.rankFill,width:`${x.wr}%`,background:i===0?"#FF6200":"#2a2a2a"}}/></div>
                  </div>
                  <div style={S.rankStat}>
                    <span style={S.rankRate}>{x.wr}%</span>
                    <span style={S.rankRecord}>{x.games}경기</span>
                  </div>
                  <span style={{fontSize:12,color:"#444"}}>›</span>
                </div>
              ))}
          ...
          <div style={S.card}>
            <div style={{...S.cardHeader,marginBottom:0}}>
              <span style={S.cardTitle}>점수</span>
              <span style={{fontSize:10,fontWeight:800,color:"#FF6200",background:"rgba(255,98,0,0.12)",padding:"2px 8px",borderRadius:4,letterSpacing:1}}>공식전 5v5</span>
            </div>
            <div style={{display:"flex",justifyContent:"center",gap:16,marginTop:16,marginBottom:6}}>
              <div style={{width:72,textAlign:"center"}}><span style={S.scoreLabel}>TEAM A</span></div>
              <div style={{width:32}}/>
              <div style={{width:72,textAlign:"center"}}><span style={S.scoreLabel}>TEAM B</span></div>
            </div>
            ...
            <div style={{display:"flex",gap:10}}>
              {(["A","B"] as const).map(t=>{
                ...
                return (
                  <div key={t} style={{flex:1,minWidth:0,borderRadius:10,border:`1px solid ${accent}80`,overflow:"hidden"}}>
                    <div style={{background:accent+"bb",padding:"8px 10px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                      <span style={{fontSize:13,fontWeight:800,color:"#fff"}}>TEAM {t}</span>
                      <span style={{fontSize:13,fontWeight:800,color:"#fff"}}>{myTeam.length}명</span>
                    </div>
                    <div style={{padding:"6px 10px",background:"#111"}}>
                      {myTeam.map(id=>{const p=players.find(x=>x.id===id);return p?(
                        <div key={id} style={{display:"flex",alignItems:"center",gap:4,padding:"9px 0",borderBottom:"1px solid #1e1e1e"}}>
                          ...
                          <PosTag pos={p.position}/>
                          <span style={{flex:1}}/>
                          <button style={{background:"none",border:"none",color:"#ef4444",cursor:"pointer",fontSize:14,fontWeight:700,padding:"0 2px",lineHeight:1}} onClick={()=>toggleTeam(t,id)}>×</button>
                        </div>
                      ):null;})}
                      {free.map(p=>(
                        <button key={p.id} style={{display:"flex",alignItems:"center",gap:6,padding:"9px 0",width:"100%",background:"transparent",border:"none",cursor:"pointer",borderBottom:"1px solid #1a1a1a"}} onClick={()=>toggleTeam(t,p.id)}>
                          ...
                          <PosTag pos={p.position}/>
                          <span style={{flex:1}}/>
                          <span style={{color:accent,fontWeight:800,fontSize:16,lineHeight:1}}>+</span>
                        </button>
                      ))}
                    </div>
                  </div>
          ...
          <div style={S.card}>
            ...
            <div style={{marginBottom:14}}>
              <div style={S.fieldLabel}>개인 득점 (선택)</div>
              <div style={S.scoresBox}>
                {allTeamPlayers.length===0&&<p style={{color:"#444",fontSize:12,textAlign:"center"}}>팀원을 먼저 추가하세요</p>}
                {allTeamPlayers.map(p=>(
                  <div key={p.id} style={S.scoreItemRow}>
                    <PosTag pos={p.position}/>
                    <span style={{flex:1,fontSize:13,fontWeight:500,color:teamA.includes(p.id)?"#FF6200":"#3b82f6"}}>{p.name}</span>
                    <input style={{...S.input,width:56,textAlign:"center",padding:"5px"}} type="number" min="0" placeholder="0"
                      value={playerScores[p.id]||""} onChange={e=>setPlayerScores(prev=>({...prev,[p.id]:e.target.value}))}/>
            ...
            {err&&<p style={{color:"#dc2626",fontSize:12,marginTop:8}}>{err}</p>}
            <button style={{...S.btnPrimary,width:"100%",padding:"16px 0",marginTop:12,fontSize:16,opacity:saving?0.6:1}} onClick={submit} disabled={saving}>
              {saving?"저장 중...":"공식전 저장"}
            </button>
          </div>
        </>
        ...
        /* 공식전 기록 로그 */
        <>
          {officialGames.length===0&&<div style={S.card}><Empty text="공식전 기록이 없습니다"/></div>}
          {(()=>{
            const grouped=officialGames.reduce((acc,g)=>{
            ...
            return keys.map(date=>(
              <div key={date} style={S.card}>
                ...
                {grouped[date].map(g=>(
                  <div key={g.id} style={S.gameWrap}>
                    <GameCard game={g} players={players} onDelete={async()=>{if(!confirm("삭제?"))return;await sb.del("games",`?id=eq.${g.id}`);onReload();}}/>
                  </div>
                ))}
              </div>
            ));
    ...
    try {
      await Promise.all(players.map(async p => {
        ...
        myGames.forEach(g => {
          ...
          if(g.winner==="D"){/* 무승부는 승패 미반영 */}
          else{const won=(inA&&g.winner==="A")||(!inA&&g.winner==="B");if(won)wins++;else losses++;}
  ...
  return (
    <div style={S.gcWrap}>
      <div style={{...S.gcMeta,position:"relative",justifyContent:"center"}}>
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          {game.game_type==='official'&&<span style={{fontSize:10,fontWeight:800,color:"#FF6200",background:"rgba(255,98,0,0.12)",padding:"2px 7px",borderRadius:4,letterSpacing:1}}>공식전</span>}
      ...
      <div style={S.gcScore}>
        <div style={{textAlign:"center",flex:1}}>
          <div style={{...S.gcTeamLabel,color:game.winner==="A"?"#FF6200":game.winner==="D"?"#60A5FA":"#555"}}>TEAM A {game.winner==="A"?"WIN":game.winner==="D"?"DRAW":""}</div>
        ...
        <div style={{textAlign:"center",flex:1}}>
          <div style={{...S.gcTeamLabel,color:game.winner==="B"?"#FF6200":game.winner==="D"?"#60A5FA":"#555"}}>TEAM B {game.winner==="B"?"WIN":game.winner==="D"?"DRAW":""}</div>
